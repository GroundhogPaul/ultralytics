...
class DFL(nn.Module):
    ...
    def __init__(self, c1=16):
        ...
        super().__init__()
        self.conv = nn.Conv2d(c1, 1, 1, bias=False).requires_grad_(False) # 不训练，只做pixelwise的加权
        # 加权的一侧是 代表c1个bin的编号0~c1; 另一侧是神经网络的输出，代表每个bin的概率

        x = torch.arange(c1, dtype=torch.float) # 代表c1个bin的编号
