...
sImgIn = "BabySmile.jpg"
inBGR_HWCh = cv2.imread(sImgIn)
inRGB_BChHW= utilBasicRunPose.read_image_BChHW(sImgIn, bNormalize=True, bSwapRB=True, bChHW=True)
inRGB_BChHW = torch.tensor(inRGB_BChHW)

# ----- Load the NCNN model and run inference ----- #
sFolderModel = "runs/LapaTrain/train_20250613_128x128/weights/best_ncnn_model"
out = []
...
cv2.imshow("out", myAnnotator.result())
cv2.waitKey(0)